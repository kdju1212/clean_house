  ...
  return (
    <div className="mt-3 flex gap-2 overflow-x-auto">
      {photoReviews.map((review) => (
        ...
        >
          {/* photoUrl is guaranteed by the filter above */}
          <Image src={review.photoUrl as string} alt="리뷰 사진" fill sizes="80px" className="object-cover" />
        </div>
      ))}
    </div>
  );
    ...
    <li className="rounded-xl border border-neutral-200 p-3">
      ...
      {review.photoUrl && (
        <div className="relative mt-2 aspect-square w-24 overflow-hidden rounded-lg bg-neutral-100">
          <Image src={review.photoUrl} alt="리뷰 사진" fill sizes="96px" className="object-cover" />
        </div>
      )}
      {reportHref && (
